...
class SearchImagery():
    def __init__(self, headers, extent):
    ...
    def init_image(self):
        ...
        payload = {'provider': 'gbdx', 'dataset': 'idaho-pansharpened', 'extent': self.extent}
        r_init = requests.post(url + initiate, data=json.dumps(payload), headers=self.headers)
    ...
    def ret_image(self):
        ...
        r_ret = requests.post(url + retrieve, data=json.dumps(payload_ret), headers=self.headers)
        r_ret_json = r_ret.json()
        self.results = r_ret_json['results']
        self.sceneIds = [self.results[i]['sceneId'] for i in range(len(self.results))]
